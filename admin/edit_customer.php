...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM users WHERE user_id = '$customer_id' AND account_type = 2";
    $result = mysqli_query($conn, $sql);
    $customer = mysqli_fetch_assoc($result);

    if (!$customer) {
        echo "<script>alert('Customer not found.'); window.location.href='customer_account.php';</script>";
        exit;
    }
} else {
    echo "<script>alert('No customer selected.'); window.location.href='customer_account.php';</script>";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE users SET first_name = '$first_name', last_name = '$last_name', contact = '$contact', address = '$address', email = '$email' $password_sql WHERE user_id = '$customer_id' AND account_type = 2";

    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Customer details updated successfully'); window.location.href='customer_account.php';</script>";
    } else {
...
<head>
    ...
    <title>ShoeMart - Edit Customer</title>
                            ...
                            <div class="card-body">
                                ...
                                <form action="edit_customer.php?id=<?php echo $customer_id; ?>" method="POST">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="contact" name="contact" value="<?php echo $customer['contact']; ?>" >
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                    ...
                                    <div class="d-flex justify-content-center">
                                       <button type="submit" class="btn btn-success mt-5">Update Customer</button> 
                                    </div>  
                                </form>
                            </div>
                          </div>
