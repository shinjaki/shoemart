...
// Check for form submission to remove a category
if (isset($_GET['remove_id'])) {
    $remove_id = $_GET['remove_id'];
    $remove_sql = "DELETE FROM category WHERE category_id = $remove_id";
    mysqli_query($conn, $remove_sql);
    header("Location: category.php"); // Refresh the page after deletion
    exit;
}
?>
                                        ...
                                        <tbody>
                                            ...
                                            if (mysqli_num_rows($result) > 0) {
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    ...
                                                    echo "<td>
                                                            <a href='category.php?remove_id=" . $row['category_id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this category?\")'>Remove</a>
                                                          </td>";
                                                    echo "</tr>";
                                                }
                                            } else {
